...
# ============================================================
# FUNGSI VALIDASI ICD-10
# ============================================================
def validate_icd10_code(code, icd_database):
    """Validasi kode ICD-10 terhadap database"""
    if not code:
...
# ============================================================
# FUNGSI EKSTRAKSI DATA DARI PDF
# ============================================================
def extract_sep_info(text):
    ...
    # Pattern untuk mencari nomor SEP - diperbarui
    sep_patterns = [
        r'No\.?SEP\s*:\s*([A-Z0-9]+)',
        r'SEP\s*:\s*([A-Z0-9]+)',
    ...
    for pattern in sep_patterns:
        ...
        if match:
            info['no_sep'] = match.group(1)
            break
    
    # Pattern untuk nama pasien - diperbarui
    nama_patterns = [
        r'Nama\s*Peserta\s*:\s*([A-Z\s]+)(?=\n|$)',
        r'Nama\s*:\s*([A-Z\s]+)(?=\n|$)',
        r'Pasien\s*:\s*([A-Z\s]+)(?=\n|$)'
    ]
    ...
    for pattern in nama_patterns:
        ...
        if match:
            info['nama_pasien'] = match.group(1).strip()
            break
    
    # Pattern untuk tanggal SEP - diperbarui
    tgl_patterns = [
    ...
    # Pattern untuk nomor kartu - diperbarui
    kartu_patterns = [
        r'No\.?Kartu\s*:\s*([0-9]+)',
        r'Kartu\s*:\s*([0-9]+)',
        r'BPJS\s*:\s*([0-9]+)'
    ...
    # Pattern untuk diagnosa (ICD-10) - DIPERBARUI untuk format yang lebih fleksibel
    diagnosa_patterns = [
        r'Diagnosa\s*Awal\s*:\s*([A-Z][0-9]{2}\.[0-9]+)\s*[-–]\s*([^\n]+)',
        r'Diagnosa\s*Awal\s*:\s*([A-Z][0-9]{2}\.[0-9]+)\s*[-]?\s*([^\n]+)',
        r'Diagnosa\s*:\s*([A-Z][0-9]{2}\.[0-9]+)\s*[-–]?\s*([^\n]+)',
        r'([A-Z][0-9]{2}\.[0-9]+)\s*[-–]\s*([A-Za-z\s,]+)'
    ]
...
def extract_rujukan_info(text):
    """Ekstrak informasi dari surat rujukan (Halaman 2)"""
    info = {
        'diagnosa_rujukan': [],
    ...
    # Pattern untuk nomor rujukan
    rujukan_patterns = [
        r'No\.?\s*Rujukan\s*:\s*([A-Z0-9\s\-]+)',
        r'Rujukan\s*:\s*([A-Z0-9\s\-]+)',
        r'Nomor\s*Rujukan\s*:\s*([A-Z0-9\s\-]+)'
    ]
    ...
    # Pattern untuk diagnosa rujukan
    diagnosa_patterns = [
        r'Diagnosa\s*:\s*([^\n]+)',
...
def extract_rekam_medis_info(text_halaman_3: str, text_halaman_4: str = "") -> Dict[str, Any]:
    """Ekstrak informasi dari rekam medis (Halaman 3) dan lanjutan (Halaman 4)"""
    ...
    nama_patterns = [
        r'Nama\s*Pasien\s*:\s*([A-Za-z\s\.]+)',
        r'Nama\s*:\s*([A-Za-z\s\.]+)',
        r'Pasien\s*:\s*([A-Za-z\s\.]+)'
    ]
    
    for pattern in nama_patterns:
        match = re.search(pattern, text_halaman_3)
        if match:
            info['nama_pasien_rm'] = match.group(1).strip()
            break
    
    # Pattern untuk diagnosa rekam medis (bisa di halaman 3 atau 4)
    diagnosa_patterns = [
    ...
    # Pattern untuk dokter DPJP (biasanya di halaman 3)
    dokter_patterns = [
...
def validate_pdf_content_detailed(pdf_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # 1. Validasi field wajib tidak boleh kosong
        if sep_info['field_missing']:
            errors.append(f"SEP field wajib kosong: {', '.join(sep_info['field_missing'])}")
        ...
        if rujukan_info['field_missing']:
            errors.append(f"Rujukan field wajib kosong: {', '.join(rujukan_info['field_missing'])}")
        ...
        if rm_info['field_missing']:
            errors.append(f"Rekam Medis field wajib kosong: {', '.join(rm_info['field_missing'])}")
        ...
        # 2. Compare field diagnosa antara SEP dan Rekam Medis
        ...
        if sep_diagnosa_codes and rm_diagnosa_codes:
            ...
            if matches:
                ...
                errors.append("Diagnosa antara SEP dan Rekam Medis tidak match")
        else:
            validation_details['diagnosa_validation']['status'] = 'INCOMPLETE_DATA'
        
        # 3. Cek tanda tangan DPJP di rekam medis
        if rm_info['tanda_tangan_dpip']:
            validation_details['signature_validation']['status'] = 'VALID'
        else:
            validation_details['signature_validation']['status'] = 'INVALID'
            errors.append("Tanda tangan DPJP tidak ditemukan di rekam medis")
        
        # 4. Validasi ICD-10 untuk semua diagnosa
        ...
        if all_invalid:
            for invalid in all_invalid:
                if invalid['kode']:
                    errors.append(f"Kode ICD-10 tidak valid: {invalid['kode']} - {invalid['validation_msg']}")
        ...
        result = {
            "valid": is_valid,
            "message": "Validasi berhasil" if is_valid else "Validasi gagal",
            "errors": errors,
            "extracted_data": {
                "sep": sep_info,
                "rujukan": rujukan_info,
...
def validate_claim_documents(extracted_files: List[str]) -> Dict[str, Any]:
    ...
    Sesuai spesifikasi: 1 file PDF harus memiliki 4 halaman (SEP, Surat Rujukan, Rekam Medis, Lanjutan Rekam Medis)
    """
    validation_errors = []
    validation_warnings = []
    valid_files = []
        ...
        if file_validation["valid"]:
            ...
            if content_validation["valid"]:
                ...
                validation_errors.append({
                    ...
                    "missing_fields": content_validation.get("missing_fields", {})
                })
        else:
            validation_errors.append({
                "filename": filename,
    ...
    # Validasi total dokumen harus sama
    if valid_files:
        total_sep = sum(1 for f in valid_files if "SEP" in f["pages_present"])
        total_rujukan = sum(1 for f in valid_files if "SURAT_RUJUKAN" in f["pages_present"])
        total_rm = sum(1 for f in valid_files if "REKAM_MEDIS" in f["pages_present"])
        
        if total_sep != total_rujukan or total_sep != total_rm:
            error_msg = f"Jumlah dokumen tidak sama: SEP={total_sep}, Rujukan={total_rujukan}, Rekam Medis={total_rm}. Harus sama jumlahnya."
            file_list = ", ".join([f["filename"] for f in valid_files])
            validation_errors.append({
                "filename": "KESELURUHAN_DATA",
                "error": f"{error_msg} File: {file_list}",
                "missing_pages": []
            })
    ...
    if validation_errors:
        return {
            "valid": False,
            "message": "Validasi struktur dokumen gagal",
            "file_errors": validation_errors,
            "valid_files": valid_files
        }
    
    return {
        "valid": True,
        "message": "Semua dokumen memiliki struktur yang valid",
        "valid_files": valid_files,
